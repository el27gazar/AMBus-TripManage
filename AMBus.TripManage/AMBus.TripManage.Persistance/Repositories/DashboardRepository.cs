    ...
    public class DashboardRepository : IDashboardRepository
    {
        ...
        public async Task<IEnumerable<PopularRouteDto>> GetPopularRoutesAsync(int top)
        {
            return await _ctx.Trips
                ...
                .Include(t => t.Reviews)
                .GroupBy(t => new { t.RouteId, t.Route.FromCity, t.Route.ToCity })
               .Select(g => new PopularRouteDto(
    g.Key.RouteId,
    g.Key.FromCity,
    g.Key.ToCity,
    g.SelectMany(t => t.Bookings).Count(b => b.Status != BookingStatus.Cancelled),
    g.SelectMany(t => t.Reviews).Any() ? g.SelectMany(t => t.Reviews).Average(r => r.Rating) : 0
)).OrderByDescending(r => r.BookingsCount).Take(top).ToListAsync();
        }
    }
}
